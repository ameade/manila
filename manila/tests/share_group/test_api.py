...
def fake_group(id, **kwargs):
    ...
    if 'source_group_snapshot_id' in kwargs:
        group['share_network_id'] = 'fake_share_network_id'
        group['share_server_id'] = 'fake_share_server_id'

    group.update(kwargs)
    return group


def fake_group_snapshot(id, **kwargs):
    snap = {
        'id': id,
        'user_id': 'fakeuser',
        'project_id': 'fakeproject',
...
class ShareGroupsAPITestCase(test.TestCase):
    def setUp(self):
        ...
        self.mock_object(db_driver, 'group_type_get',
                         mock.Mock(return_value=self.fake_group_type))

    def test_create_empty_request(self):
        group = fake_group('fakeid',
                           user_id=self.context.user_id,
                           project_id=self.context.project_id,
                           status=constants.STATUS_CREATING)

        expected_values = group.copy()
    ...
    def test_create_with_name(self):
        ...
        db_driver.share_group_create.assert_called_once_with(
            self.context, expected_values)
        self.scheduler_rpcapi.create_share_group.assert_called_once_with(
            self.context, group_id=group['id'], request_spec=mock.ANY,
            filter_properties={}
        )
    ...
    def test_create_with_multiple_share_types(self):
        ...
        self.api.create(self.context,
                        share_type_ids=fake_share_type_ids)
    ...
    def test_create_with_source_group_snapshot_id(self):
        snap = fake_group_snapshot("fake_source_group_snapshot_id",
                                   status=constants.STATUS_AVAILABLE)
        fake_share_type_mapping = {'share_type_id': self.fake_share_type['id']}
        orig_group = fake_group('fakeorigid',
                                user_id=self.context.user_id,
                                project_id=self.context.project_id,
        ...
        for name in ('id', 'created_at', 'share_network_id',
                     'share_server_id'):
            expected_values.pop(name, None)
        expected_values['source_group_snapshot_id'] = snap['id']
        expected_values['share_types'] = [self.fake_share_type['id']]
        expected_values['share_network_id'] = 'fake_network_id'
        expected_values['share_server_id'] = 'fake_server_id'

        self.mock_object(db_driver, 'group_snapshot_get',
                         mock.Mock(return_value=snap))
        self.mock_object(db_driver, 'share_group_get',
                         mock.Mock(return_value=orig_group))
        self.mock_object(db_driver, 'share_group_create',
                         mock.Mock(return_value=group))
        self.mock_object(db_driver, 'share_get',
                         mock.Mock(return_value=stubs.stub_share(
                             'fake_share')))
        self.mock_object(share_types, 'get_share_type',
                         mock.Mock(return_value={
                             "id": self.fake_share_type['id']}))
        self.mock_object(db_driver, 'share_network_get')
        self.mock_object(db_driver, 'group_snapshot_members_get_all',
                         mock.Mock(return_value=[]))

        self.api.create(self.context,
                        source_group_snapshot_id=snap['id'])

        db_driver.share_group_create.assert_called_once_with(
            self.context, expected_values)
        self.share_rpcapi.create_share_group.\
            assert_called_once_with(self.context, group, orig_group['host'])

    def test_create_with_source_group_snapshot_id_with_member(self):
        snap = fake_group_snapshot("fake_source_group_snapshot_id",
                                   status=constants.STATUS_AVAILABLE)
        share = stubs.stub_share('fakeshareid')
        member = stubs.stub_group_snapshot_member('fake_member_id')
        fake_share_type_mapping = {'share_type_id': self.fake_share_type['id']}
        orig_group = fake_group('fakeorigid',
                                user_id=self.context.user_id,
                                project_id=self.context.project_id,
    ...
    def test_create_with_source_group_snapshot_id_with_members_error(self):
        snap = fake_group_snapshot("fake_source_group_snapshot_id",
                                   status=constants.STATUS_AVAILABLE)
        member = stubs.stub_group_snapshot_member('fake_member_id')
        member_2 = stubs.stub_group_snapshot_member('fake_member2_id')
        share = stubs.stub_share('fakeshareid')
        fake_share_type_mapping = {'share_type_id': self.fake_share_type['id']}
        orig_group = fake_group('fakeorigid',
                                user_id=self.context.user_id,
    ...
    def test_create_with_source_group_snapshot_id_error_snapshot_status(self):
        snap = fake_group_snapshot("fake_source_group_snapshot_id",
                                   status=constants.STATUS_ERROR)
        self.mock_object(db_driver, 'group_snapshot_get',
                         mock.Mock(return_value=snap))

        self.assertRaises(exception.InvalidGroupSnapshot, self.api.create,
                          self.context, source_group_snapshot_id=snap['id'])

    def test_create_with_source_group_snapshot_id_snap_not_found(self):
        snap = fake_group_snapshot("fake_source_group_snapshot_id",
                                   status=constants.STATUS_ERROR)
        self.mock_object(db_driver, 'group_snapshot_get',
                         mock.Mock(side_effect=exception.GroupSnapshotNotFound(
                             group_snapshot_id='fake_source_group_snapshot_id'
                         )))

        self.assertRaises(exception.GroupSnapshotNotFound, self.api.create,
                          self.context, source_group_snapshot_id=snap['id'])
    ...
    def test_delete_creating_no_host(self):
        ...
        self.api.delete(self.context, group)
    ...
    def test_delete_creating_with_host(self):
        ...
        self.assertRaises(exception.InvalidShareGroup, self.api.delete,
                          self.context, group)
    ...
    def test_delete_available(self):
        group = fake_group('fakeid',
                           ...
                           status=constants.STATUS_AVAILABLE,
                           host="fake_host")
        deleted_group = copy.deepcopy(group)
        deleted_group['status'] = constants.STATUS_DELETING
        self.mock_object(db_driver, 'share_group_update',
                         mock.Mock(return_value=deleted_group))
        ...
        db_driver.share_group_update.assert_called_once_with(
            self.context, group['id'], {'status': constants.STATUS_DELETING})
        self.share_rpcapi.delete_share_group.assert_called_once_with(
            self.context, deleted_group
        )
    ...
    def test_delete_with_group_snapshots(self):
        group = fake_group('fakeid',
                           user_id=self.context.user_id,
                           project_id=self.context.project_id,
                           status=constants.STATUS_AVAILABLE,
                           host="fake_host")
        self.mock_object(db_driver, 'count_group_snapshots_in_share_group',
                         mock.Mock(return_value=1))
    ...
    def test_update_no_values(self):
        ...
        self.api.update(self.context, group, expected_values)

        db_driver.share_group_update.assert_called_once_with(
            self.context, group['id'], expected_values)
    ...
    def test_get(self):
        expected_group = fake_group('fakeid',
                                    user_id=self.context.user_id,
                                    project_id=self.context.project_id,
                                    status=constants.STATUS_CREATING)
        self.mock_object(db_driver, 'share_group_get',
                         mock.Mock(return_value=expected_group))

        actual_group = self.api.get(self.context, expected_group['id'])
    ...
    def test_get_all(self):
        expected_groups = [fake_group('fakeid',
                                      user_id=self.context.user_id,
                                      project_id=self.context.project_id,
                                      status=constants.STATUS_CREATING)]
        self.mock_object(db_driver, 'share_group_get_all_by_project',
                         mock.Mock(return_value=expected_groups))

        actual_group = self.api.get_all(self.context, detailed=True)
        self.assertEqual(expected_groups, actual_group)
    ...
    def test_get_all_all_tenants_not_admin(self):
        ...
        actual_groups = self.api.get_all(cxt,
    ...
    def test_get_all_all_tenants_as_admin(self):
        ...
        actual_groups = self.api.get_all(self.context,
                                         search_opts={'all_tenants': True})
        self.assertEqual(expected_groups, actual_groups)
        db_driver.share_group_get_all.assert_called_once_with(
            self.context, detailed=True, filters={'all_tenants': True},
            sort_dir=None, sort_key=None)

    def test_create_group_snapshot_minimal_request_no_members(self):
        group = fake_group('fake_group_id',
                           user_id=self.context.user_id,
                           project_id=self.context.project_id,
                           status=constants.STATUS_AVAILABLE)
        snap = fake_group_snapshot('fakeid',
                                   user_id=self.context.user_id,
                                   project_id=self.context.project_id,
                                   share_group_id=group['id'],
                                   status=constants.STATUS_CREATING)
        ...
        self.mock_object(db_driver, 'share_group_get',
                         mock.Mock(return_value=group))
        self.mock_object(db_driver, 'group_snapshot_create',
                         mock.Mock(return_value=snap))
        self.mock_object(db_driver, 'share_get_all_by_share_group_id',
                         mock.Mock(return_value=[]))

        self.api.create_group_snapshot(self.context,
                                       share_group_id=group['id'])

        db_driver.share_group_get.assert_called_once_with(
            self.context, group['id']
        )
        db_driver.group_snapshot_create.assert_called_once_with(
            self.context, expected_values)
        self.share_rpcapi.create_group_snapshot.assert_called_once_with(
            self.context, snap, group['host']
        )

    def test_create_group_snapshot_minimal_request_no_members_with_name(self):
        fake_name = 'fake_name'
        group = fake_group('fake_group_id',
                           user_id=self.context.user_id,
                           project_id=self.context.project_id,
    ...
    def test_create_group_snapshot_group_does_not_exist(self):
        group = fake_group('fake_group_id',
                           user_id=self.context.user_id,
                           project_id=self.context.project_id,
                           status=constants.STATUS_CREATING)
        ...
        self.assertRaises(exception.InvalidShareGroup,
                          self.api.create_group_snapshot,
                          self.context,
                          share_group_id=group['id'])
    ...
    def test_create_group_snapshot_group_in_creating(self):
        self.mock_object(db_driver, 'share_group_get',
                         mock.Mock(
                             side_effect=exception.ShareGroupNotFound(
                                 share_group_id='fake_id'
                             )))

        self.assertRaises(exception.ShareGroupNotFound,
                          self.api.create_group_snapshot,
                          self.context,
                          share_group_id="fake_id")
    ...
    def test_create_group_snapshot_with_member(self):
        group = fake_group('fake_group_id',
                           user_id=self.context.user_id,
                           project_id=self.context.project_id,
                           status=constants.STATUS_AVAILABLE)
        ...
        self.mock_object(db_driver, 'group_snapshot_create',
                         mock.Mock(return_value=snap))
        self.mock_object(db_driver, 'group_snapshot_member_create',
                         mock.Mock())
        self.mock_object(db_driver, 'share_get_all_by_share_group_id',
                         mock.Mock(return_value=[share]))
        ...
        db_driver.group_snapshot_create.assert_called_once_with(
            self.context, expected_values)
        db_driver.group_snapshot_member_create.assert_called_once_with(
    ...
    def test_create_group_snapshot_with_member_share_in_creating(self):
        group = fake_group('fake_group_id',
                           user_id=self.context.user_id,
                           project_id=self.context.project_id,
                           status=constants.STATUS_AVAILABLE)
    ...
    def test_create_group_snapshot_with_two_members(self):
        group = fake_group('fake_group_id',
                           user_id=self.context.user_id,
                           project_id=self.context.project_id,
                           status=constants.STATUS_AVAILABLE)
        ...
        db_driver.group_snapshot_member_create.assert_any_call(
    ...
    def test_create_group_snapshot_error_creating_member(self):
        group = fake_group('fake_group_id',
                           user_id=self.context.user_id,
                           project_id=self.context.project_id,
                           status=constants.STATUS_AVAILABLE)
        ...
        self.mock_object(db_driver, 'group_snapshot_create',
                         mock.Mock(return_value=snap))
        self.mock_object(db_driver, 'group_snapshot_destroy')
        self.mock_object(db_driver, 'group_snapshot_member_create',
                         mock.Mock(side_effect=exception.Error))
        self.mock_object(db_driver, 'share_get_all_by_share_group_id',
                         mock.Mock(return_value=[share]))

        self.assertRaises(exception.Error, self.api.create_group_snapshot,
                          self.context, share_group_id=group['id'])
        ...
        db_driver.group_snapshot_destroy.assert_called_once_with(
            self.context, snap['id']
        )

    def test_delete_group_snapshot(self):
        group = fake_group('fake_id', host="fake_host")
        snap = fake_group_snapshot('fake_groupsnap_id',
                                   share_group_id='fake_id',
                                   status=constants.STATUS_AVAILABLE)
        self.mock_object(db_driver, 'share_group_get',
                         mock.Mock(return_value=group))
        self.mock_object(db_driver, 'group_snapshot_update')

        self.api.delete_group_snapshot(self.context, snap)

        db_driver.share_group_get.assert_called_once_with(
            self.context, "fake_id"
        )
        db_driver.group_snapshot_update.assert_called_once_with(
            self.context, snap['id'], {'status': constants.STATUS_DELETING}
        )
        self.share_rpcapi.delete_group_snapshot.assert_called_once_with(
            self.context, snap, group['host'])

    def test_delete_group_snapshot_group_does_not_exist(self):
        snap = fake_group_snapshot('fake_groupsnap_id',
                                   share_group_id='fake_id')
        self.mock_object(db_driver, 'share_group_get',
                         mock.Mock(
        ...
        self.assertRaises(exception.ShareGroupNotFound,
                          self.api.delete_group_snapshot,
                          self.context,
                          snap)
    ...
    def test_delete_group_snapshot_creating_status(self):
        snap = fake_group_snapshot('fake_groupsnap_id',
                                   share_group_id='fake_id',
                                   status=constants.STATUS_CREATING)
        self.mock_object(db_driver, 'share_group_get')
    ...
    def test_update_group_snapshot_no_values(self):
        snap = fake_group_snapshot('fakeid',
                                   user_id=self.context.user_id,
                                   project_id=self.context.project_id,
                                   status=constants.STATUS_CREATING)
        ...
        self.api.update_group_snapshot(self.context, snap, expected_values)

        db_driver.group_snapshot_update.assert_called_once_with(
            self.context, snap['id'], expected_values)

    def test_update_group_snapshot_with_name(self):
        fake_name = 'fake_name'
        snap = fake_group_snapshot('fakeid',
                                   user_id=self.context.user_id,
                                   project_id=self.context.project_id,
    ...
    def test_group_snapshot_get(self):
        expected_snap = fake_group_snapshot(
        ...
        actual_group = self.api.get_group_snapshot(self.context,
                                                   expected_snap['id'])
        self.assertEqual(expected_snap, actual_group)

    def test_group_snapshot_get_all_no_groups(self):

        self.mock_object(db_driver, 'group_snapshot_get_all',
                         mock.Mock(return_value=[]))

        actual_group = self.api.get_all_group_snapshots(self.context)
        self.assertEqual([], actual_group)

    def test_group_snapshot_get_all(self):
        expected_snaps = [fake_group('fakeid',
                                     user_id=self.context.user_id,
                                     project_id=self.context.project_id,
                                     status=constants.STATUS_CREATING)]
        self.mock_object(db_driver, 'group_snapshot_get_all_by_project',
                         mock.Mock(return_value=expected_snaps))

        actual_group = self.api.get_all_group_snapshots(self.context,
                                                        detailed=True)
        self.assertEqual(expected_snaps, actual_group)

    def test_group_snapshot_get_all_all_tenants_not_admin(self):
        cxt = context.RequestContext(user_id=None,
                                     project_id=None,
                                     is_admin=False)
        expected_snaps = [fake_group('fakeid',
        ...
        actual_groups = self.api.get_all_group_snapshots(
            cxt, search_opts={'all_tenants': True})
        self.assertEqual(expected_snaps, actual_groups)

    def test_group_snapshot_get_all_all_tenants_as_admin(self):
        expected_snaps = [fake_group('fakeid',
                                     user_id=self.context.user_id,
                                     project_id=self.context.project_id,
                                     status=constants.STATUS_CREATING)]
        ...
        self.assertEqual(expected_snaps, actual_groups)
        db_driver.group_snapshot_get_all.assert_called_once_with(
            self.context, detailed=True, filters={'all_tenants': True},
            sort_dir=None, sort_key=None)

    def test_get_all_group_snapshot_members(self):
        self.mock_object(db_driver, 'group_snapshot_members_get_all',
                         mock.Mock(return_value=[]))

        self.api.get_all_group_snapshot_members(self.context, 'fake_id')

        db_driver.group_snapshot_members_get_all.assert_called_once_with(
            self.context, 'fake_id'
        )
