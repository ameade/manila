...
class API(base.Base):
    ...
    def create(self, context, share_proto, size, name, description,
               ...
               share_group_id=None, group_snapshot_member=None):
        """Create new share."""
        ...
        if share_group_id:
            try:
                share_group = self.db.share_group_get(
                    context, share_group_id)
            except exception.NotFound as e:
                raise exception.InvalidParameterValue(six.text_type(e))

            if (not group_snapshot_member and
                    not (share_group['status'] ==
            ...
            if share_type_id:
                group_st_ids = [st['share_type_id'] for st in
                                share_group.get('share_types', [])]
                if share_type_id not in group_st_ids:
                    params = {
                        'type': share_type_id,
        ...
        if group_snapshot_member:
            options['source_group_snapshot_member_id'] = group_snapshot_member[
                'id']
        ...
        self.create_instance(context, share, share_network_id=share_network_id,
                             ...
                             group_snapshot_member=group_snapshot_member)
    ...
    def create_instance(self, context, share, share_network_id=None,
        ...
        if group_snapshot_member:
            # Inherit properties from the group_snapshot_member
            member_share_instance = group_snapshot_member['share_instance']
            updates = {
            ...
            share = self.db.share_instance_update(context,
                                                  share_instance['id'],
                                                  updates)
            # NOTE(ameade): Do not cast to driver if creating from group
            # snapshot
            return
    ...
    def _create_share_instance_and_get_request_spec(
        ...
        share_properties = {
            ...
            'source_group_snapshot_member_id': share[
    ...
    def delete(self, context, share, force=False):
        ...
        group_snapshot_members_count = (
            self.db.count_group_snapshot_members_in_share(context, share_id))
        if group_snapshot_members_count:
            msg = (_("Share still has %d dependent group snapshot members") %
                   group_snapshot_members_count)
            raise exception.InvalidShare(reason=msg)
    ...
    def delete_share_server(self, context, server):
        ...
        groups = self.db.share_group_get_all_by_share_server(context,
                                                             server['id'])
        if groups:
