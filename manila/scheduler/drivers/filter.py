...
class FilterScheduler(base.Scheduler):
    ...
    def _format_filter_properties(self, context, filter_properties,
        ...
        group = request_spec.get('share_group')
    ...
    def schedule_create_share_group(self, context, group_id,
                                    request_spec,
                                    filter_properties):

        LOG.info(_LI("Scheduling share group %s") % group_id)

        host = self._get_best_host_for_share_group(context, request_spec)

        if not host:
            msg = _("No hosts available for share group %s") % group_id
            raise exception.NoValidHost(reason=msg)

        msg = _LI("Chose host %(host)s for create_share_group %(group)s")
        LOG.info(msg % {'host': host, 'group': group_id})

        updated_group = base.group_update_db(context, group_id, host)

        self.share_rpcapi.create_share_group(context, updated_group, host)
    ...
    def _get_weighted_hosts_for_group_type(self, context, request_spec,
                                           group_type):
        config_options = self._get_configuration_options()
        all_hosts = self.host_manager.get_all_host_states_share(context)
    ...
    def _get_weighted_candidates_group(self, context, request_spec):
        ...
        for iteration_count, share_type in enumerate(shr_types):
            ...
            # can support all share types of the group
        ...
        # NOTE(ameade): Ensure the hosts support the group type
        group_type = request_spec.get("resource_type", {})
        temp_weighed_group_hosts = self._get_weighted_hosts_for_group_type(
            elevated, request_spec, group_type)
        new_weighed_hosts = []
        for host1 in weighed_hosts:
