...
def upgrade():
    ...
    # Add share group extra specs
    try:
        op.create_table(
            'share_group_type_extra_specs',
            ...
            sql.Column('deleted', sql.String(length=36)),
            sql.Column('share_group_type_id', sql.String(36),
                       sql.ForeignKey('share_group_types.id',
                                      name="sgtp_id_extra_specs_fk")),
            mysql_engine='InnoDB',
        )
    except Exception:
    ...
    try:
        op.create_table(
            ...
            sql.UniqueConstraint('share_group_type_id', 'project_id',
                                 'deleted', name="sgtp_project_id_uc"),
            mysql_engine='InnoDB',
        )
    except Exception:
    ...
    op.create_table(
        ...
        sql.Column('share_group_type_id', sql.String(length=36),
                   sql.ForeignKey('share_group_types.id',
                                  name="sgtp_id_sgt_id_uc"),
                   nullable=False),
        sql.Column('share_type_id', sql.String(length=36),
                   sql.ForeignKey('share_types.id',
                                  name="sgtp_id_st_id_uc"),
                   nullable=False),
        mysql_engine='InnoDB',
        mysql_charset='utf8')
...
def downgrade():
    ...
    # Drop mappings
    try:
        op.drop_table('share_group_type_share_type_mappings')
    except Exception:
        LOG.error(_LE("share_group_type_share_type_mappings table"
                      " not dropped"))
        raise

    try:
        op.drop_table('share_group_type_projects')
    except Exception:
        LOG.error(_LE("share_group_type_projects table not dropped"))
        raise

    try:
        op.drop_table('share_group_type_extra_specs')
    except Exception:
        LOG.error(_LE("share_group_type_extra_specs table not dropped"))
        raise

    try:
        op.drop_table('share_group_types')
    except Exception:
        LOG.error(_LE("share_group_types table not dropped"))
        raise
