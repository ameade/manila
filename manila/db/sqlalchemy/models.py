...
class Share(BASE, ManilaBase):
    ...
    source_group_snapshot_member_id = Column(String(36), nullable=True)
...
class ShareGroupTypes(BASE, ManilaBase):
    """Represent possible share_group_types of volumes offered."""
...
class ShareGroup(BASE, ManilaBase):
    ...
    status = Column(String(255))
    source_group_snapshot_id = Column(String(36))
    share_network_id = Column(String(36), ForeignKey('share_networks.id'),
                              nullable=True)
    share_server_id = Column(String(36), ForeignKey('share_servers.id'),
                             nullable=True)
    share_group_type_id = Column(String(36),
                                 ForeignKey('share_group_types.id'),
                                 nullable=True)
...
class ShareGroupTypeProjects(BASE, ManilaBase):
    """Represent projects associated share_group_types."""
    ...
    id = Column(Integer, primary_key=True)
    share_group_type_id = Column(String, ForeignKey('share_group_types.id'),
                                 nullable=False)
    project_id = Column(String(255))
...
class ShareGroupTypeExtraSpecs(BASE, ManilaBase):
    """Represents additional specs for a share_group_type."""
    __tablename__ = 'share_group_type_extra_specs'
    id = Column(Integer, primary_key=True)
    key = Column("spec_key", String(255))
    value = Column("spec_value", String(255))
    share_group_type_id = Column(String(36),
    ...
    share_group_type = orm.relationship(
        ...
        foreign_keys=share_group_type_id,
        primaryjoin='and_('
        'ShareGroupTypeExtraSpecs.share_group_type_id == ShareGroupTypes.id,'
        'ShareGroupTypeExtraSpecs.deleted == 0)'
    )


class GroupSnapshot(BASE, ManilaBase):
    ...
    share_group = orm.relationship(
        ...
        primaryjoin=('and_('
                     'GroupSnapshot.share_group_id == ShareGroup.id,'
                     'GroupSnapshot.deleted == "False")')
    )
...
class GroupSnapshotMember(BASE, ManilaBase):
    """Represents the share snapshots in a share group snapshot."""
    __tablename__ = 'group_snapshot_members'
    id = Column(String(36), primary_key=True)
    group_snapshot_id = Column(String(36), ForeignKey('group_snapshots.id'))
    share_id = Column(String(36), ForeignKey('shares.id'))
    share_instance_id = Column(String(36), ForeignKey('share_instances.id'))
    size = Column(Integer)
    status = Column(String(255))
    ...
    group_snapshot = orm.relationship(
        GroupSnapshot,
        backref="group_snapshot_members",
        foreign_keys=group_snapshot_id,
        primaryjoin='GroupSnapshot.id == '
                    'GroupSnapshotMember.group_snapshot_id')
